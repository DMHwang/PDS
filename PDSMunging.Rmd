...
sum(is.na(file_df$xrsa_flux))/length(file_df$xrsa_flux)
```
...
```{r}
l1 <- min(merge_df_18_peaks$time)
l2 <- min(merge_df_17_peaks$time)
u1 <- max(merge_df_18_peaks$time)
u2 <- max(merge_df_17_peaks$time)
...
```{r}
merge_list <- rbind(merge_df_18_peaks, merge_df_17_peaks)
duplicate_merge_peaks <- which(duplicated(merge_list))
...
duplicate_merge_peaks <- which(duplicated(df_merge_peaks))
...
# Merge the dataframes based on time with priority
result_df <- bind_rows(t1, t2) %>%
  arrange(ascd(time))

result_df <- full_join(merge_df_13_peaks, merge_df_18_peaks, by = join_by("time")) 
